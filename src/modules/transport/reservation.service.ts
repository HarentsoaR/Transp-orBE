...
export const ReservationService = {
  async create(input: ReservationInput, userId?: string) {
    ...
    if (trip.availableSeats < uniqueSeats.length) throw new Error("Pas assez de places disponibles");

    const overCapacitySeat = uniqueSeats.find((s) => s > trip.bus.capacity);
    if (overCapacitySeat) throw new Error(`Seat ${overCapacitySeat} exceeds bus capacity`);
  ...
  async cancel(reservationId: string, actorId?: string) {
    const reservation = await prisma.reservation.findUnique({ where: { id: reservationId } });
    if (!reservation) throw new Error("Reservation not found");
    if (reservation.status === ReservationStatus.CANCELLED) return reservation;

    const seatCount = await prisma.reservationSeat.count({ where: { reservationId } }).then((c) => (c > 0 ? c : 1));
